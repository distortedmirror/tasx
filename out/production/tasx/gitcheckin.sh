...
if [ "$del" != "y" -a "$del" != "Y" ]; then
	for i in `find . -name '*.decrypted'`; do echo rm $i; done
	for i in `find . -name '*.unencrypted'`; do echo rm $i; done
fi
echo 'Replace new encrypted XML to main XML(y/n)?'
read repl
if [ "$repl" != "y" -a "$repl" != "Y" ]; then
	for i in `find . -name '*.encrypted'`; do echo cp $i `echo $i|perl -pe 's/.encrypted//g'`; done
fi
