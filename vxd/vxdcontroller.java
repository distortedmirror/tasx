...
public class vxdcontroller {
    ...
    public void configOpenedXML(Element rootEl,vxdDropTarget target){
        loadingFile=true;
        NodeList mainEls=rootEl.getChildNodes();
		...
		if(elname.equals("Connection")){
		    Runnable r=new Runnable(){
			    Element el;
			    public void setArgs(Element el){this.el=el;}
			    public void run(){
				vxdJConnectorButton conn=GetIconConnectorButton("Connection");
				conn.loadedElement=el;
				addConnectorStart(conn,0,0,iconConnectionView.getDragIconByID(el.getAttribute("SourceID")));
				addConnectorDrop(iconConnectionView.getDragIconByID(el.getAttribute("SourceID")),iconConnectionView.getDragIconByID(el.getAttribute("DestID")),vxdIconConnector.CONNECTION);
			    }
			};
		    ...
		    SwingUtilities.invokeLater(new Runnable(){public void run() { vxd.controller.refreshXMLViews();}});
		    ...
		    vxdJButton icon=GetIconButton(elname);
		    icon.loadedElement=el;
		    vxdDropTarget ptarget=vxd.controller.addIcon(icon,fp.x,fp.y,target);
		    if(ptarget instanceof vxdDragIcon)
			((vxdDragIcon)ptarget).setLocation(x,y);
		    SwingUtilities.invokeLater(new Runnable(){public void run() { vxd.controller.refreshXMLViews();}});
		    configOpenedXML(el,ptarget);
		}
		//NodeList childEls=nd.getChildNodes();
                //initOpenedXMLTree(childEls,parentPoint);
            }
        }
	SwingUtilities.invokeLater(new Runnable(){public void run() {iconConnectionView.validateIconsAndConnectors(); vxd.controller.refreshXMLViews();}});
    }
    ...
    public void DropSourceIconToDropTarget(String sourceName,
	    ...
	    // pt.y+=vxd.ydragsize/2;
            ...
            Runnable r=new Runnable() {
                ...
                public void run() {
		    Point fp=evt.getPoint();//SwingUtilities.convertPoint(source,e.getPoint(),vxd.frame);
		    vxd.controller.addIcon(source,fp.x,fp.y,(vxdDropTarget)SwingUtilities.
			getDeepestComponentAt(vxd.controller.iconConnectionView,fp.x,fp.y));
                    if(vxd.DEBUG)
    ...
    public vxdJConnectorButton GetIconConnectorButton(String name) {
        vxdJConnectorButton toolButton = null;
        try {
            for(int j=0;j<((Container)vxd.toolBarPanel.getComponent(1)).getComponentCount();++j) {
                Component buttonSearch=
                        ((Container)vxd.toolBarPanel.getComponent(1)).getComponent(j);
                
                if(buttonSearch instanceof vxdJConnectorButton) {
                    
                    if(((vxdJConnectorButton)buttonSearch).name.trim().compareTo(name)==0) {
                        toolButton = (vxdJConnectorButton)buttonSearch;
                        return toolButton;
                    }
                }
            }
...
/*		}else{
    ...
    public vxdDropTarget addIcon(vxdJButton icon,int x,int y,vxdDropTarget target) {
	if(target!=iconConnectionView) {
	    x+=vxd.xdragsize;
	    y+=vxd.ydragsize;
	}
	vxdDragIcon dragicon=new vxdDragIcon(icon,x,y,null);
        synchronized(iconLoadedElementSync) {
            ...
            dragicon=new vxdDragIcon(icon,x,y,null);
    ...
    public void addConnectorDrop(vxdDragIcon source,vxdDragIcon target,int type) {
        if(type==vxdIconConnector.CONNECTION) {
            Element el=null;
	    if(loadingFile) {
		vxdJConnectorButton icon=GetIconConnectorButton("Connection");
                el=project.programXML.createElement("Connection");
                for(int xi=0;xi<icon.loadedElement.getAttributes().getLength();++xi) {
                    String attrn=icon.loadedElement.getAttributes().item(xi).getNodeName();
                    el.setAttribute(attrn,icon.loadedElement.getAttribute(attrn));
                }
            } else {
		el=showElementPropertyBox("Connection",project.programXML,-1,-1);
	    }
            if(el!=null) {
